...
$(document).ready(function(){
    ...
    $(document).on('click', '.view', function(){
        ...
        $.ajax({
            ...
            success:function(data){
                $('#productModal').modal('show');
                $('#category_id').val(data.category_id).prop('disabled', true);;
                $('#brand_id').html(data.brand_select_box).prop('disabled', true);;
                $('#brand_id').val(data.brand_id); 
                $('#product_name').val(data.product_name).prop('disabled', true);;
                $('#product_description').val(data.product_description);
                $('#newinput').append("<input type='text' id='nowinput' class='form-control' required pattern='[+-]?([0-9]*[.])?[0-9]+'/>"); 
