...
if(isset($_POST['btn_action']))
{
	...
	{
		$query = "
		INSERT INTO inventory_order (user_id, inventory_order_total,tax, inventory_order_date, inventory_order_name, inventory_order_address, payment_status, inventory_order_status, inventory_order_created_date) 
		VALUES (:user_id, :inventory_order_total,:tax, :inventory_order_date, :inventory_order_name, :inventory_order_address, :payment_status, :inventory_order_status, :inventory_order_created_date)
		";
		...
		if(isset($inventory_order_id))
		{
			...
			for($count = 0; $count<count($_POST["product_id"]); $count++)
			{
				...
				$base_cost = $product_details['tax'] * $_POST["quantity"][$count];
				$total_cost= $total_cost + $base_cost;
				$total_amount = $total_amount + $base_price;
			}
			$update_query = "
			UPDATE inventory_order 
			SET inventory_order_total = '".$total_amount."',tax='".$total_cost."' 
