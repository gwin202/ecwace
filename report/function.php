...
function count_total_transfer_order_value($connect)
{
	$query = "
	SELECT sum(inventory_order_total) as total_order_value FROM inventory_order WHERE payment_status = 'transfer' AND inventory_order_status='active'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= ' AND user_id = "'.$_SESSION["user_id"].'"';
...
function count_total_donation_order_value($connect)
{
	$query = "
	SELECT sum(inventory_order_total) as total_order_value FROM inventory_order WHERE payment_status = 'donation' AND inventory_order_status='active'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= ' AND user_id = "'.$_SESSION["user_id"].'"';
...
function get_user_wise_total_order($connect)
{
	...
	SUM(CASE WHEN inventory_order.payment_status = "transfer" THEN inventory_order.inventory_order_total ELSE 0 END) AS transfer_order_total,
	SUM(CASE WHEN inventory_order.payment_status = "donation" THEN inventory_order.inventory_order_total ELSE 0 END) AS donation_order_total, 
	...
	<div class="table-responsive">
		<table class="table table-bordered table-striped">
			<tr>
				...
				<th>Total Transfer Order</th>
				<th>Total Donation Order</th>
			</tr>
	';
	...
	$total_transfer_order=0;
	$total_donation_order= 0;
	foreach($result as $row)
	{
		$output .= '
		<tr>
			<td>'.$row['user_name'].'</td>
			<td align="right"><strike>N</strike>'.number_format($row["order_total"],2).'</td>
			<td align="right"><strike>N</strike> '.number_format($row["cash_order_total"],2).'</td>
			<td align="right"><strike>N</strike> '.number_format($row["credit_order_total"],2).'</td>
			<td align="right"><strike>N</strike> '.number_format($row["transfer_order_total"],2).'</td>
			<td align="right"><strike>N</strike> '.number_format($row["donation_order_total"],2).'</td>
		</tr>
		';
		...
		$total_transfer_order=$total_transfer_order+$row["transfer_order_total"];
		$total_donation_order=$total_donation_order + $row["donation_order_total"];
	}
	$output .= '
	<tr>
		<td align="right"><b>Total</b></td>
		<td align="right"><b><strike>N</strike> '.number_format($total_order,2).'</b></td>
		<td align="right"><b><strike>N</strike> '.number_format($total_cash_order,2).'</b></td>
		<td align="right"><b><strike>N</strike>'.number_format($total_credit_order,2).'</b></td>
		<td align="right"><b><strike>N</strike>'.number_format($total_transfer_order,2).'</b></td>
		<td align="right"><b><strike>N</strike> '.number_format($total_donation_order,2).'</b></td>
	</tr></table></div>
	';
	return $output;
}
